...
const Sidebar = () => {
  ...
  const logOutHandler = async () => {
    /* console.log(session!.backendTokens.access_token);
    while (!session || !session.backendTokens || !session.backendTokens.access_token) {
  ...
  return (
    <div
      className={`p-5 pt-8 min-h-86vh dark:bg-very_dark border-r border-r-gray-900 ${
        open ? "w-72" : "w-20"
    ...
    >
      ...
      <button
        className={`flex justify-center items-center rounded-md mt-6 dark:bg-light w-full hover:bg-blue-100 hover:border-blue-500 dark:hover:bg-medium border border-black dark:border-none ${
          !open ? "px-2.5" : "px-4"
        } py-2`}
        onClick={() => router.push("/chat-homepage")}
      >
        <FiPlusCircle
      ...
      <ul
        className="pt-2 overflow-y-auto max-h-[500px] overflow-x-hidden"
        style={{
          scrollbarWidth: "thin",
      ...
      >
        {Menus.map((menu: any, index: number) => (
          <React.Fragment key={index}>
            <li
              className={`dark:text-gray-300 text-sm flex items-center gap-x-4 cursor-pointer p-2 border hover:bg-blue-100 border-gray-100 dark:border-none hover:border-blue-500 dark:hover:bg-medium rounded-md ${
